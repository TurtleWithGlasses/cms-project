...
def test_retrieve_user_profile():
    ...
    login_response = client.post("/token", data={"username": "newuser@example.com", "password": "newuserpassword"})
...
def test_update_user_profile():
    ...
    # Update user profile
    update_response = client.patch("/users/me", headers={"Authorization": f"Bearer {token}"}, json={
...
def test_delete_user_by_admin():
    ...
    login_response = client.post("/token", data={"username": "admin@example.com", "password": "adminpassword"})
    admin_token = login_response.json()["access_token"]

    # Delete the created user
    delete_response = client.delete("/users/2", headers={"Authorization": f"Bearer {admin_token}"})
